...
def load_grayscale_image(image_path: str) -> np.ndarray:
    """Load an image from disk and return a float32 grayscale array."""
    image = Image.open(image_path).convert("L")
    return np.asarray(image, dtype=np.float32)


def prewitt_kernels() -> Tuple[np.ndarray, np.ndarray]:
    """Return Prewitt X and Y gradient kernels."""
    kernel_x = np.array(
        [[-1, 0, 1],
         [-1, 0, 1],
         [-1, 0, 1]],
...
def _make_worker(
    ...
    """Create a worker that applies a convolution kernel to a row range."""
    kernel_height, kernel_width = kernel.shape
    out_height, out_width = output.shape

    def worker(row_start: int, row_end: int) -> None:
        """Compute convolution results for rows [row_start, row_end)."""
...
def multi_threaded_convolution(
    ...
    """Apply convolution on a 2D image using multiple threads."""
    kernel_height, kernel_width = kernel.shape
...
def save_grayscale_image(image: np.ndarray, output_path: str) -> None:
    """Save a grayscale numpy array to disk."""
    Image.fromarray(image, mode="L").save(output_path)


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for Prewitt edge detection."""
    parser = argparse.ArgumentParser(
        description="Apply Prewitt edge detection using threaded convolution.",
    )
...
def main() -> None:
    """Run the Prewitt edge detection pipeline."""
    args = parse_args()
...
def run_prewitt_edge(image_path: str) -> np.ndarray:
    threads = 8
